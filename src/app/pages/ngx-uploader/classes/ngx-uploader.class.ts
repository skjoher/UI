    ...
    return input.subscribe((event: UploadInput) => {
      switch (event.type) {
        ...
        case 'uploadAll':
          ...
          files.forEach(file => {
            //TO-DO Calculate hash and put it in file.hash
            var reader = new FileReader();
            var localUploadScheduler = this.uploadScheduler;
            reader.onload = function () {
              var testing = new jsSHA("SHA-1","ARRAYBUFFER");
              testing.update(reader.result);
              var testHash = testing.getHash("HEX");
              file.hash = testHash;
              console.log(file.hash);
              localUploadScheduler.next({ file: file, event: event });
            }
  ...
  uploadFile(file: UploadFile, event: UploadInput): Observable<UploadOutput> {
    return new Observable(observer => {
      ...
      try {
        ...
        file.form.append('sha1-'+file.hash,file.nativeFile);
        
        //file.form.append(file.hash || 'file', uploadFile, uploadFile.name);
        //console.log(file.form.getAll);
